...
object KMeans {
  ...
  def print_help() {
    ...
    println("   <String \"m\" for two stages implementation, else single stage>")
  }
  ...
  def main(args: Array[String]) {
    ...
    var (kDimension          : Int,
         ...
         kStage              : String) =
    args.length match {
      case 7 => (args(0).toInt, args(1).toInt, args(2).toInt, args(3).toInt, args(4).toFloat, args(5).toInt, args(6))
    ...
    for (i <- 1 to kIterationNum) {
      if (kStage != "m") {
        // implementation with only one tage
        println("***** implementation with a single stage")
        ...
        // implementation with two stages
        println("***** implementation with two stages, spin wait is disabled in this case")
        val newC = points.map( p => (closestPoint(p, centers), (p, 1)))
                         .reduceByKey{ case ((p1, c1), (p2, c2)) => (vectorAdd(p1, p2), c1 + c2)}
                         .map( pair => (pair._1, vectorScale(pair._2._1, (1.0 / pair._2._2))))
                         .collectAsMap()
